...
protocol ONTOPageControlDelegate: AnyObject {
    func pageControl(_ pageControl: ONTOPageControl, didSelectPageAt index: Int)
}

extension ONTOPageControlDelegate {
    func pageControl(_ pageControl: ONTOPageControl, didSelectPageAt index: Int) { }
}

class ONTOPageControl: UIControl {
    ...
    weak var delegate: ONTOPageControlDelegate?
    ...
    private func generateDotView() -> UIView {
        ...
        if let dotImage = dotImage {
            ...
            dotView = ONTOAnimateDotView(frame: CGRect(x: 0, y: 0, width: dotSize.width, height: dotSize.height))
            if let d = dotView as? ONTOAnimateDotView {
    ...
    func changeActivity(activity: Bool, at index: Int) {
        if let d = dots[index] as? ONTOAnimateDotView {
